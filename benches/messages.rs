...
async fn call_actor(num: u64) -> Duration {
    ...
    for _ in 0..num {
        ...
        handles.push(tokio::spawn(async move {
            // TODO: Don't use tokio::spawn, put future into vec
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("Call actors", |b| {
        ...
        b.to_async(&rt)
            .iter_custom(|messages| black_box(call_actor(messages)));
    });
    ...
    group.bench_function("Message passing rust channels", |b| {
        b.iter_custom(move |messages| black_box(send_messages_through_channels(messages)))
    });

    group.bench_function("Message passing async_channels blocking", |b| {
        let rt = tokio::runtime::Runtime::new().unwrap();
        b.to_async(&rt)
            .iter_custom(|messages| black_box(async_channels_blocking(messages)))
    });

    group.bench_function("Message passing async_channels parallel A", |b| {
        let rt = tokio::runtime::Runtime::new().unwrap();
        b.to_async(&rt)
            .iter_custom(|messages| black_box(async_channels_parallel_A(messages)))
    });
    ...
    group.bench_function("tokio mpsc", |b| {
        let rt = tokio::runtime::Runtime::new().unwrap();
        b.to_async(&rt)
            .iter_custom(|messages| black_box(tokio_mpsc_parallel(messages)));
    });

    group.bench_function("tokio mpsc blocking", |b| {
        let rt = tokio::runtime::Runtime::new().unwrap();
        b.to_async(&rt)
            .iter_custom(|messages| black_box(tokio_mpsc_blocking(messages)));
    });
}
